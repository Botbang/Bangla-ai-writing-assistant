...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, onSave, onClear, currentApiKey }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-2xl p-8 max-w-md w-full m-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3">
            ...
            <button
              onClick={onClear}
              className="flex-1 px-4 py-3 bg-red-800 hover:bg-red-700 text-white font-bold rounded-lg shadow-md transition-colors"
            >
              Clear Key & Sign Out
            </button>
        </div>
